...
def get_footprints(latitude, longitude):
    ...
    Returns:
        np.array: Moisture flow of the evaporation footprint for the specified location (source cell).
    """
    ...
    # Retrieve the moisture flow (cubic meters) associated to the evaporation footprint for the specified source cell
...
evaporation_footprint = np.zeros((360, 720))
evaporation_footprint = get_footprints(lat, lon)

# Optional: Plotting results (customize as needed)
plt.figure(figsize=(10, 6))
plt.imshow(evaporation_footprint, extent=[lons.min(), lons.max(), lats.min(), lats.max()])
plt.colorbar(label="Moisture Flow Volume [m$^3$]")
plt.title(f"Destination of evaporation for Location (Lat: {lat}, Lon: {lon})")
